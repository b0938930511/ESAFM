...
mkdir ${tech[$i]}/obj
    ...
    merge_end=$(date +%s%N) 
    $BC -x ir benchmark_all_link_tmp.bc $OPT -Os -fPIC -fno-semantic-interposition -fvisibility-inlines-hidden -ffunction-sections -fdata-sections -fno-common -o ${tech[$i]}/obj/llvm_$j.o -c
    all_end=$(date +%s%N)
    echo "$((($merge_end-$merge_start)/1000000))" >> ${tech[$i]}/time/time.txt
    echo "$((($all_end-$merge_start)/1000000))" >> ${tech[$i]}/time/all_time.txt
    stat ${tech[$i]}/obj/llvm_$j.o |awk {'print $2'} |  grep -v ${tech[$i]}/obj/llvm_$j.o | head -n 1 >> size.txt 
    tail -n1 < ${tech[$i]}/err/err$j.txt >> ${tech[$i]}/Merge_Count/Merge_count.txt
    #$BC ${tech[$i]}/obj/llvm_$j.o $OPT -fuse-ld=$LLD -o ${tech[$i]}/exe/llvm_$j -Wl,--export-dynamic -lpthread  -lrt  -ldl  -lpthread  -lm  /usr/lib/x86_64-linux-gnu/libz.so  /usr/lib/x86_64-linux-gnu/libtinfo.so
    done
